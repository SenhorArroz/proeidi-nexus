							...
							>
								...
								<div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
									...
									<div className="flex shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-white">
										{formulario.publicado && <><Link href={`/questionarios/${formulario.slug}`} target="_blank" className="grid min-h-11 min-w-11 place-items-center bg-sky-100 text-sky-800 transition hover:bg-sky-200" aria-label="Abrir questionário" title="Abrir questionário"><ExternalLink className="h-4 w-4" /></Link><button type="button" onClick={() => setQuestionarioQr({ titulo: formulario.titulo, slug: formulario.slug })} className="grid min-h-11 min-w-11 place-items-center border-l border-slate-200 bg-sky-100 text-sky-800 transition hover:bg-sky-200" aria-label="Gerar QR Code" title="Gerar QR Code"><QrCode className="h-4 w-4" /></button></>}
										<Link href={`/nexus/diretoria/questionarios/${formulario.id}`} className="grid min-h-11 min-w-11 place-items-center border-l border-slate-200 bg-orange-100 text-orange-800 transition hover:bg-orange-200" aria-label="Ver estatísticas" title="Estatísticas"><BarChart3 className="h-4 w-4" /></Link>
										<Link href={`/nexus/diretoria/formularios?id=${formulario.id}`} className="grid min-h-11 min-w-11 place-items-center border-l border-slate-200 bg-sky-100 text-sky-800 transition hover:bg-sky-200" aria-label="Editar questionário" title="Editar"><Pencil className="h-4 w-4" /></Link>
										<button type="button" onClick={() => setConfirmarExclusao({ id: formulario.id, titulo: formulario.titulo, respostas: formulario._count.respostas })} className="grid min-h-11 min-w-11 place-items-center border-l border-slate-200 bg-red-100 text-red-800 transition hover:bg-red-200" aria-label="Excluir questionário" title="Excluir"><Trash2 className="h-4 w-4" /></button>
									</div>
								</div>
							</article>
						))
