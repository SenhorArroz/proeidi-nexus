...
export function DiretoriaPageIntro({ title, description, icon: Icon, actions }: DiretoriaPageIntroProps) {
	return (
		<header className="diretoria-page-intro">
			...
			<div className="relative flex min-w-0 max-w-full flex-1 items-start gap-3 sm:gap-4">
			...
			{actions && <div className="relative mt-4 flex w-full min-w-0 flex-wrap gap-2 sm:mt-0 sm:w-auto sm:shrink-0">{actions}</div>}
		</header>
	);
}
