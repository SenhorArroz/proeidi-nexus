...
function validarConfirmacao(input: z.infer<typeof passwordInput>) {
	if (input.novaSenha !== input.confirmacao) throw new TRPCError({ code: "BAD_REQUEST", message: "A confirmação de senha não confere." });
}
...
export const contaRouter = createTRPCRouter({
	senhaObrigatoria: publicProcedure.query(async ({ ctx }) => {
		...
		const [user] = await ctx.db.$queryRaw<Array<{ senhaAlteradaEm: Date | null }>>`
	...
	alterarMinhaSenha: protectedProcedure.input(passwordInput).mutation(async ({ ctx, input }) => {
	...
	solicitarRedefinicao: directorProcedure.input(z.object({ usuarioId: z.string().cuid() })).mutation(async ({ ctx, input }) => {
		const [solicitante, destinatario] = await Promise.all([
			ctx.db.user.findUnique({ where: { id: ctx.session.user.id }, select: { role: true } }),
			ctx.db.user.findUnique({ where: { id: input.usuarioId }, select: { id: true, nome: true, email: true, role: true } }),
		]);
		if (!solicitante || !destinatario) throw new TRPCError({ code: "NOT_FOUND", message: "Pessoa não encontrada." });
		if (solicitante.role === "DIRETOR" && !["PROFESSOR", "MONITOR"].includes(destinatario.role)) throw new TRPCError({ code: "FORBIDDEN", message: "Diretores só podem solicitar redefinição para professores e monitores." });
		...
		const [recente] = await ctx.db.$queryRaw<Array<{ id: string }>>`
			...
			LIMIT 1
		`;
		if (recente) throw new TRPCError({ code: "TOO_MANY_REQUESTS", message: "Aguarde um minuto antes de enviar outro código." });
		...
		try {
			...
			await sendPasswordResetEmail({ to: destinatario.email, nome: destinatario.nome, codigo, url });
		} catch (error) {
			await ctx.db.$executeRaw`UPDATE "PasswordResetRequest" SET "usedAt" = NOW() WHERE "id" = ${requestId}`;
			throw new TRPCError({ code: "PRECONDITION_FAILED", message: error instanceof Error ? error.message : "Não foi possível enviar o e-mail." });
		}
		return { ok: true };
	}),
	confirmarRedefinicao: publicProcedure.input(z.object({ solicitacaoId: z.string().cuid(), codigo: z.string().regex(/^\d{6}$/, "Informe o código de seis dígitos."), ...passwordInput.shape })).mutation(async ({ ctx, input }) => {
		validarConfirmacao(input);
		const [request] = await ctx.db.$queryRaw<Array<{ id: string; userId: string; codigoHash: string }>>`
			SELECT "id", "userId", "codigoHash" FROM "PasswordResetRequest"
			WHERE "id" = ${input.solicitacaoId} AND "usedAt" IS NULL AND "expiresAt" >= NOW()
			LIMIT 1
		`;
		if (!request || !(await verifyPassword(input.codigo, request.codigoHash))) throw new TRPCError({ code: "BAD_REQUEST", message: "O código é inválido, expirou ou já foi utilizado." });
		const senhaHash = await hashPassword(input.novaSenha);
		await ctx.db.$transaction(async (tx) => {
			await tx.$executeRaw`
