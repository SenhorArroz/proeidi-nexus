...
export const diretorRouter = createTRPCRouter({
	...
	update: coordinatorProcedure.input(personInput.extend({ id })).mutation(async ({ ctx, input }) => {
		...
		return ctx.db.user.update({ where: { id: input.id }, data: { nome: input.nome, email: input.email.toLowerCase(), matricula: input.matricula }, select: { id: true, updatedAt: true } });
	}),
